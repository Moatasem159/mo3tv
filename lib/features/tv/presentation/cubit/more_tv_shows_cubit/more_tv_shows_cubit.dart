...
class MoreTvShowsCubit extends Cubit<MoreTvShowsStates> {
  MoreTvShowsCubit(this._getTvShowsListUsecase) : super(MoreTvShowsInitialState()){
    page=1;
    moreTvShows=[];
  }
  ...
  seeMoreTvShows({required int index}) async {
    ...
    page++;
    if (index == 1) {
      response = await _getTvShowsListUsecase.call(MediaParams(page: page, lang: AppStrings.appLang,mediaType: AppStrings.tv,listType: AppStrings.trending));
    } else if (index == 2) {
      response = await _getTvShowsListUsecase.call(MediaParams(page: page, lang: AppStrings.appLang,mediaType: AppStrings.tv,listType: AppStrings.popular));
    } else if (index == 3) {
      response = await _getTvShowsListUsecase.call(MediaParams(page: page, lang: AppStrings.appLang,mediaType: AppStrings.tv,listType: AppStrings.topRated));
    }
