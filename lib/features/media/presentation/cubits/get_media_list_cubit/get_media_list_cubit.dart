...
class GetMediaListCubit extends HydratedCubit<GetMediaListStates> {
  GetMediaListCubit(this._getMediaListUsecase,this.params) : super(GetMediaListInitialState());
   ...
   if(state is!GetMediaListSuccessState)
     {
      emit(GetMediaListLoadingState());
       Either<Failure,List<Media>> response =await _getMediaListUsecase.call(MediaParams(lang: AppStrings.appLang,mediaType: params.mediaType,listType: params.listType));
       emit(response.fold(
               (failure)=>GetMediaListErrorState(msg: mapFailureToMsg(failure)),
               (media)=>GetMediaListSuccessState(media,DateTime.now().toIso8601String(),AppStrings.appLang)));
     }
  }
  @override
